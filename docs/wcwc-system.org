...
The Wire-Cell team has developed a system to cluster computers for the purpose of sharing hardware, software and human resources with a focus on Wire-Cell related development.  It is given the name Wire-Cell Workstation Cluster (WCWC) as it is composed primarily of team members' workstations.

This document describes the WCWC in some detail.  It assumes the audience is the author/developer of WCWC (in the future when memory fades), others that wish to implement WCWC on their computers or just anyone interested in all the details.  Users who simply want to know the basics of how they can join and take advantage of WCWC may wish to focus on the [[file:wcwc-user.org]] document and come back to this document if they wish to learn more.  Many of the specific details are coded into the ~wcwc~ program that is available from https://github.com/wirecell/wcwc.  Many of the examples below will utilize this program.
...
To get started, a workstation owner should follow these steps:

1. Install Debian stable (currently 12 aka "bookworm") on your workstation.
2. Run the following as root (see section [[NFS Client]] for more info):
   #+begin_example
   apt install nfs-common
   mkdir /wcwc
   mount -t nfs lycastus.phy.bnl.gov:/ro/wcwc /wcwc
   #+end_example
3. As a normal user, and assuming bash shell:
   #+begin_example
   ...
   #+end_example
4. Optional, give the WCWC admin(s) SSH access to the ~root~ user of your workstation.
   #+begin_example
   sudo mkdir -p /root/.ssh   # if needed
   sudo chmod 700 /root/.ssh  # if needed
   curl http://lycastus.phy.bnl.gov/~wcwc/wcwc.pub | sudo tee -a /root/.ssh/authorized_keys
   #+end_example
   This URL is internal to the BNL wired campus network.  
See the [[file:wcwc-user.org]] document for more details.
