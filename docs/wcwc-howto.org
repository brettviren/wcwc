...
* Use Spack View to develop WC Prototype
:PROPERTIES:
:CUSTOM_ID: spack-view-wcp
:END:

Developing Wire-Cell Prototype is very similar to Wire-Cell Toolkit except for some work arounds due to some issues in WCP's older build system.
...
In general, you should follow section [[#create-view]] on creating a "view" above but with following differences:

- Replace ~wire-cell-toolkit~ with ~wire-cell-prototype~.
- Do not use ~$PREFIX~ as the installation prefix for WCP itself.
- Add the installation prefix to ~PATH~-like variables in the ~.envrc~ file.



#+begin_example
$ wcwc view -S wirecell -d wcp-check -s "wire-cell-prototype" wcp-check/local
$ git clone --recurse git@github.com:BNLIF/wire-cell.git wcp-check/prototype
$ echo 'export WCP_PREFIX=$PWD/install' >> wcp-check.envrc
$ echo 'load_prefix $PWD/install' >> wcp-check.envrc
$ echo 'path_add LD_LIBRARY_PATH $PWD/install/lib64' >> wcp-check.envrc
$ cd wcp-check/prototype
$ direnv allow

$ python --version  # note first two digits of version for --with-python-include

$ ./waf-tools/waf configure \
    --prefix=$WCP_PREFIX \
    --boost-mt --boost-libs=$PREFIX/lib --boost-includes=$PREFIX/include \
    --with-eigen=$PREFIX \
    --with-glpk=$PREFIX \
    --with-python=$PREFIX \
    --with-python-include=$PREFIX/include/python3.11
$ ./waf-tools/waf --notests
$ ./waf-tools/waf --notests install

$ wire-cell-<TAB>
$ wire-cell-eloss
#+end_example

Omit ~--notests~ to build and run all tests.  The full ~.envrc~ file is:

#+begin_example
# envrc generated by wcwc, edits may be lost.  
# view add -i /nfs/data/1/bviren/wcp-check/local prototype
# 
...
export WCP_PREFIX=$PWD/install
load_prefix $PWD/install
path_add LD_LIBRARY_PATH $PWD/install/lib64
#+end_example
...
To use WCP's DNN features we must make some Python-related changes.  First, 
edit ~.envrc~ to add a line:

#+begin_example
path_add PYTHONPATH $PWD/wcp-check/prototype/pyinf
export TMPDIR=$PWD/tmp
export TEMP=$PWD/tmp
#+end_example

Setting the temporary directory variables anticipates the next step where we
install some Python packages that are too large to use the default ~/tmp~.  Any
path on a large and fast disk is fine.

The Python packages will be installed into a virtual environment provided by
~direnv~.  The required packages are driven by use of [[https://github.com/facebookresearch/SparseConvNet?tab=readme-ov-file#setup][SparseConvNet]]:
...
$ cd ..   # back to top of direnv-controlled working area
#+end_example
Notes:
- This will use about 5GB of disk space under ~.direnv/~.
- As of today the above installs PyTorch 2.5.0+cu124.  
- This installs SCN 

A quick test:
#+begin_example
$ python -c 'import sparseconvnet as scn; print(scn.__file__)'
#+end_example
This should print a path under ~.direnv/~.
...
#+begin_note
The software to run container images and the images themselves may be taken from CVMFS.  This requires the ~/cvmfs~ mount.  There can be a delay up to a minute the first time an area in CVMFS is accessed after some time.  Be patient.  Subsequent access is typically very fast.
#+end_note

** Quick start
:PROPERTIES:
:CUSTOM_ID: container-quick-start
:END:
...
#+begin_note
On BNL WCWC you may see the message ~WARNING: group: unknown groupid 22351~ (with your group ID).  It can safely be ignored.
#+end_note

** Choosing a container image
:PROPERTIES:
:CUSTOM_ID: container-image-choice
:END:


The ~wcwc container~ is really a simplified version of the ~apptainer~ command. You may specify the container image through a nick name, a relative or absolute path or a URL.   Known nick names can be listed:
#+begin_example
...
* Stand-alone Spack builds
:PROPERTIES:
:CUSTOM_ID: stand-alone-spack
:END:


Most users will not require this but the admins and some users may want to have
their own Spack install.  Admins should not attempt this with the ~wcwc~ account.
Here, we use the example of developing the [[https://github.com/wirecell/wire-cell-spack][wire-cell-spack]] builder.

First time:
#+begin_example
cd /nfs/data/1/bviren/standalone
git clone \
  -c feature.manyFiles=true --depth 1 --branch v0.23.1 \
  https://github.com/spack/spack.git 
mkdir scope
cp spack/etc/spack/defaults/config.yaml scope/
#+end_example
See [[https://github.com/spack/spack/releases/][Spack releases]] for a suitable version.  Start a subshell: 
#+begin_example
bash --rcfile spack/share/spack/setup-env.sh
export SPACK_USER_CONFIG_PATH=$(pwd)/scope
spack config blame
#+end_example
This last checks that the configuration is using your custom scope and not mixing with WCWC scopes that any mixing with your default user scope under =~/.spack/= is acceptable.

Now, we add the Spack repo that we want to develop:
#+begin_example
git clone git@github.com:WireCell/wire-cell-spack.git wct-spack
spack repo add $(pwd)/wct-spack
#+end_example
And install the target.
#+begin_example
spack install wire-cell-toolkit@master+glpk+hdf+root+torch cuda_arch=89
#+end_example
