...
** OS packages
:PROPERTIES:
:CUSTOM_ID: os-packages
:END:

The BNL WCWC admin assures workstations have a common set of select OS packages to support ~wcwc~ user, developer and admin tasks.  In addition, generally useful packages may be installed across all workstations.  Users are welcome to contact the WCWC admin to request additional OS packages.  Section [[#ansible]] describes the mechanism for assuring these OS packages.
