...
Enrollment is required to allow users to share hardware and software.  Enrolling a workstation in the BNL WCWC requires the computer custodian to perform these tasks or request the BNL WCWC admins for assistance:
- Notify the WCWC admins of the intention to enroll the workstation and discuss any special needs.
- Install an OS supported by the BNL WCWC (currently Debian 12 / "bookworm").
...
Every BNL WCWC user must perform these tasks to gain access to WCWC workstations:
- Assure they have a [[https://www.phy.bnl.gov/Accounts/][Physics department SSH account]].
- The user has a primary account on an existing WCWC workstation or host WCWC workstation can be identified.
- Coordinate with the WCWC admin to integrate any existing local accounts on WCWC workstations.

The user account integration process requires the user to briefly log out of a WCWC workstation while the account and its files are given a unique user ID number.  If the user has more than one primary account, one must be selected to provide ~$HOME~ to the WCWC cluster.
...
** Spack packages and ~wcwc~

The large software stacks for various projects and experiments are installed by the WCWC admin via Spack and made available to each WCWC workstation under the ~/wcwc~ directory.   The ~wcwc~ program is available to simplify usage of this software.  It is placed in the same location as the ~spack~ program itself.
