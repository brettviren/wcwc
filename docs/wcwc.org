...
- Section [[#nabs]] documents how ~wcwc~ works under the hood to automate various tasks.
...
Commands:
  ...
  env     Create a user Spack Environment in a directory.
...
wcwc admin builder
#+end_example
This command is idempotent and an existing ~wcwc-builder~ container will not be overwritten by default.  In rare circumstances the image must be updated.  The existing image may be deleted and the above command rerun or a rebuild can be forced:
#+begin_example
...
When a new ~/wcwc~ area is brought to life, perhaps by specifying a noncanonical prefix via ~wcwc --prefix /path/to/wcwc~, a one-time bootstrap process is required.  Currently this requires manual, interactive commands to be typed in the builder container.
...
#+end_example
Observe that ~spack bootstrap now~ causes clingo and other things to be downloaded and installed.

Renter the container one more time to check:
#+begin_example
$ wcwc --prefix=$HOME/wcwc-dev admin shell
root@19e414c431e5:/# spack spec zlib
#+end_example
Observe that this time no bootstrap messages are shown.  
...
#+begin_note
In section [[#podman-builder]] the use of a podman container image to install packages is introduced.  In the container the ~/wcwc~ path is always used.  Outside the container the ~WCWC_PREFIX~ or ~--prefix~ value is that of a host directory.  It will be mounted onto ~/wcwc~ in the container. 
#+end_note


To begin, we use ~wcwc~ to produce an initial configuration file named ~wcwc.yaml~ in the prefix directory:
#+begin_example
$ wcwc admin defaults
#+end_example
The ~wcwc.yaml~ file provides high level parameters that govern how ~wcwc~ works.  Most should be left as-is with the exception of potentially changing the ~tag~ for one or more stack.  This can be needed if the Spack repo for the stack has had new releases for which ~wcwc~ has not seen updates.
...
Instead of editing ~wcwc.yaml~ you may set/change configuration parameters by running ~init~ with various options.  For example, to use the current main branch for Spack:
#+begin_example
