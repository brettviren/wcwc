...
This document describes the Wire-Cell workstation cluster (WCWC).  Sections [[#enroll]], [[#base]] and [[#stacks]] are intended to provide easy entry points to WCWC users (including using WCWC for software development). Section [[#spack]] gives a summary of information about Spack that is most salient to users of WCWC and developers of WCWC infrastructure.  Section [[#further]] provides guidance on optional ways to use WCWC.  The section [[#nab]] gives details on various parts of how WCWC is constructed and works.  And finally the section [[#sysadmin]] describes use of Ansible to do OS-level sysadmin things for computers enrolled in a WCWC.
...
As described here, WCWC instance resides on the BNL campus network.  It is comprised of a subset of the workstations used by the BNL Wire-Cell team.  To enroll a workstation into WCWC the owner must satisfy these requirements:
...
A workstation may then be integrated into a WCWC in a variety of ways.  One main outcome is that the directory
#+begin_example
/wcwc
#+end_example
becomes available.  This directory provides all WCWC software.
...
The OS level of WCWC workstations is managed by Ansible using recipes provided by the ~wcwc~ package.  See section [[#direnv-wcwc]] for the recommended way to use this package.  It assumes [[https://docs.ansible.com/ansible/latest/reference_appendices/config.html][Ansible-related environment]] as defined in ~wcwc/.envrc~.

** Ansible setup
:PROPERTIES:
:CUSTOM_ID: ansible-setup
:END:

See [[https://docs.ansible.com/][Ansible docs]] for details on Ansible.  Here we give the highlights.  We refer to ~admin~ as a user on an Ansible [[https://docs.ansible.com/ansible/latest/getting_started/][control node]].  It is ~admin~ that runs Ansible commands.

The configuration starts with a list of hosts (~wcwc/ansible/inventory.cfg~)
#+begin_example
  [workstations]
  hierocles.bnl
  # etc ...
#+end_example
Hostnames are fully qualified but are intended to match stanzas in an SSH configuration file and not necessarily DNS entries.  After adding a new host, check basic SSH connectivity allows login with no interruption for authentication (ie, ~ssh-agent~ key management is working).
#+begin_example
ssh root@hierocles.bnl
#+end_example
Then check Ansible configuration
#+begin_example
ansible-inventory --list
ansible-inventory --host hierocles.bnl
ansible workstations -m ping -l hierocles.bnl
#+end_example
